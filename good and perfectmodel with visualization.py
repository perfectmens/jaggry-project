...
If a user asks for a batch range
automatically translate it to a safe SQL SELECT query in the form:


Never generate INSERT, UPDATE, or DELETE queries.
After fetching, compute delay = next_batch_start - end_time 
and classify each batch as Perfect, Normal, Cleaning Delay, or Failure.
Then produce visualizations automatically.

You are an analytical assistant for a jaggery manufacturing process. 
The data has the following columns:
batch_id, start_time, end_time, next_batch_start

There is no column like 'status' or 'gap_delay'. 
You must calculate the time gap (delay) mathematically using:
delay = next_batch_start - end_time

Convert all times to seconds or minutes. 
If time crosses midnight (e.g., 59:49 → 00:09), treat it as the next day by adding 24 hours before subtraction.

Then, classify each batch based on the delay:
- delay ≤ 10 seconds → Perfect
- 10 < delay ≤ 60 seconds → Normal
- 60 < delay ≤ 900 seconds → Cleaning Delay
- delay > 900 seconds → Failure

Finally, show:
1. batch_id with computed delay and category
2. Average delay per category
3. The longest and shortest delay batches
4. Simple visualizations like bar chart (batch vs delay) or pie chart (category distribution)

Important: 
- Always derive the category from the calculated delay (do not assume labels exist)
- Be time-format tolerant (with or without date)
- If time decreases, assume next day continuation
- Output clear text summary and visuals depending on user query

you need to find gap_delay (string: 'normal', 'failure', 'perfect', or 'cleaning_delay').
"""

# ▼▼▼ MODIFIED SQL PROMPT (Stricter) ▼▼▼
...
- **CRITICAL: ONLY generate the SQL query.**
- **DO NOT** output any text, explanation, notes, or markdown *except* for the single SQL query.
- ONLY output SELECT queries. Never use UPDATE, DELETE, DROP, etc.
- When filtering by date/time, use standard 'YYYY-MM-DD HH:MM:SS' format for timestamps (e.g., `start_time >= '2025-10-22 02:59:00'`).
- If the user asks for analysis of a range (e.g., "analyze batch 1 to 20"), generate the query for that range (e.g., "SELECT * FROM batch_data WHERE batch_id BETWEEN 1 AND 20;").

User Question: "{{question}}"
SQL Query:
"""
# ▲▲▲ END MODIFIED SQL PROMPT ▲▲▲
...
# Streamlit UI
# -------------------------------
st.set_page_config(page_title="🏭 AURALIS Jaggery Assistant", layout="centered")
st.title("🏭 Jaggery Batch Assistant")
st.caption(f"Conversational interface for TimescaleDB, powered by ({MODEL_NAME}).under Apache 2.0 licence")
...
def generate_sql(question):
    ...
    # Final check for any lingering explanation text
...
# ▼▼▼ MODIFIED Analysis & Plotting Function ▼▼▼
# -------------------------------
...
def perform_analysis_and_plot(df_input):
    ...
    if 'batch_id' in df.columns:
        summary += f"Analysis for Batch IDs {df['batch_id'].min()} to {df['batch_id'].max()}\n\n"
    ...
    if 'batch_id' in df.columns:
        summary += "Batches by Category:\n"
        for category in labels:
            batches_in_category = df[df['delay_category'] == category]['batch_id']
            count = len(batches_in_category)
            if count == 0:
                summary += f"- {category}: 0 batches\n"
            elif count > 20: # Don't list 1000s of IDs
                summary += f"- {category}: {count} batches (e.g., IDs from {batches_in_category.min()} to {batches_in_category.max()})\n"
            else:
                summary += f"- {category}: {count} batches (IDs: {', '.join(batches_in_category.astype(str).tolist())})\n"
        summary += "\n"
    ...
    with st.chat_message("assistant"):
        with st.spinner("Thinking..."):
            ...
            if "error" in df.columns:
                ...
                if is_analysis_request and 'batch_id' in df.columns: # Ensure batch_id is present for analysis
                    ...
                    results_for_llm = analysis_summary
                     ...
                     results_for_llm = "Could not perform analysis as 'batch_id' column is missing from the query result. Please rephrase."
                else:
